...
function Login() {
    ...
    useEffect(() => {
        if (auth.status === 201) {
            ...
            navigate('/')
        } else if (auth.status === 222) {
            toast.error(auth.data.error)
        }
    }, [auth.data]);
    ...
    return (
        <div className=' w-full h-screen flex items-center flex-col bg-gray-500 p-4 lg:p-10'>
            ...
            <form onSubmit={LoginHanlder} className='max-w-[500px] mt-10 w-full flex flex-col gap-y-6 bg-white rounded-2xl shadow-lg p-6 md:p-9'>

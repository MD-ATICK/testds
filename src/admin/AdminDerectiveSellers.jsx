...
function AdmindeactiveSellers() {

    const [loading, setloading] = useState(true);
    const [deactiveSellers, setdeactiveSellers] = useState([]);

    const deactiveSellersFetch = async () => {
        const token = localStorage.getItem('ds-token')
        const { data, status } = await api.get('/v4/get-deactive-sellers', { headers: { Authorization: `Bearer ${token}` } })
        if (status === 200) {
            setloading(false)
            setdeactiveSellers(data.sellers)
            console.log(data)
        }
    }

    useEffect(() => {
        deactiveSellersFetch()
    }, []);

    return (
        <MainLayout>
            <div className=' h-full relative w-full bg-white md:bg-gray-200 py-1 md:px-4 md:py-3 overflow-x-scroll scroll-smooth'>
                {
                    loading &&
                ...
                <div className='fixed md:static left-0 pr-7 pl-4 w-[100vw] md:w-full mb-4 md:px-10 mt-3 flex items-center gap-x-4 justify-between'>
                    <div className='flex items-center gap-x-2'>
                        ...
                        <select name="" id="" className=' bg-gray-200 md:bg-white p-1 px-2 rounded-md'>
                    ...
                    <div className=' flex items-center gap-x-6'>
                        <div className='h-[40px] flex justify-end items-center'>
                            <input type="text" className=' bg-gray-200 md:bg-white outline-none rounded-lg w-[200px] md:w-[300px] focus:w-full lg:focus:w-[500px] duration-500 h-full placeholder:text-stone-500 text-[14px] px-4' placeholder='Search' />
                ...
                <div className='w-[800px] md:w-full h-[80vh] bg-white rounded-2xl p-1 shadow-lg mt-14 md:mt-0'>
                    <div className='w-full relative px-2 overflow-x-scroll scroll-smooth'>
                        <div className='w-full bg-white pb-6 pt-3 rounded-2xl mt-0'>
                            ...
                            {
                                !loading && deactiveSellers && deactiveSellers.length > 0 ?
                                    deactiveSellers.map((ds, i) => {
                                        const { _id, avatar, name, email, paymentStatus, status } = ds
                                        return <div key={_id} className='h-[550px] md:h-[450px] overflow-y-scroll flex flex-col gap-y-5 bggra rounded-2xl p-4'>
                                            <div className=' w-full flex rounded-2xl px-1 items-center '>
                                                ...
                                                <p className=' md:font-sans josefin text-stone-700 md:font-[600] md:border-r tracking-wider text-center pl-3 border-gray-100 text-[15px] md:text-[14px] flex-[0.7] md:flex-[1.2] flex items-center justify-center'><img className='h-9 w-9 object-cover rounded-md border border-stone-300' src={avatar} alt="" /></p>
                                                ...
                                                <p className=' md:font-sans josefin text-stone-700 md:font-[600] md:border-r tracking-wide text-[15px] md:text-[14px] pl-3 text-center border-gray-300 flex-[2]'>{email}</p>
                                                {
                                                    paymentStatus === 'active' ?
                                                        ...
                                                        :
                                                        <p className='  text-stone-700  tracking-wider flex justify-center items-center border-gray-400 text-[14px] flex-[1.2]'><span className=' py-1 px-3 rounded-lg text-[13px] border-[2px] border-red-500 bg-[#f4d3cc]'>{paymentStatus}</span></p>
                                                }
                                                <p className='  text-stone-700  tracking-wider flex justify-center items-center border-gray-400 text-[14px] flex-[1.2]'><span className=' py-1 px-3 rounded-lg text-[13px] border-[2px] border-purple-600 bg-[#dbccf4]'>{status}</span></p>
                                                <Link to={`/admin/seller/details/${_id}`} className=' md:font-sans josefin text-stone-700 md:font-[600]  tracking-wide text-[15px] md:text-[14px] pl-3 text-center border-gray-300 flex-[1] flex justify-center '><AiOutlineEye className='bg-green-600 text-white text-[32px] p-[6px] rounded-md shadow-md' /></Link>
                                            </div>
                                        </div>
                                    })
                                    :
